...
export interface ProgressiveFluxLoaderProps {
   ...
   * flux gradient. Recolor the default via the `--flux-from` / `--flux-to` CSS
   * variables, or pass any CSS background to replace it.
   */
  gradient?: string
...
// Signature "flux" palette — a vivid blue → cyan → blue fill. End colors come
// from CSS variables with built-in defaults, so the bar can be recolored per
// instance (set `--flux-from` / `--flux-to`) or replaced via the `gradient` prop.
...
// Colored glow from the flux palette, a white top-edge highlight, and a
// deep-blue inset for depth.
...
// White sweep over the fill (blended with `screen`) so the highlight reads as a
// bright glide.
...
function pickLabel(value: number, sortedPhases: ProgressiveFluxPhase[]): string {
